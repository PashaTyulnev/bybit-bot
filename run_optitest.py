"""
OptiTest – 100-Coin Multi-Backtest (standalone, ohne Streamlit).

Führt durch:
  1. Top-100 USDT-Perp Symbole von Bybit laden (nach Volumen)
  2. Fehlende Coins herunterladen (365 Tage, 15m)
  3. Alle Strategien × Hebel backtesten, bestes Setup pro Coin wählen
  4. Ergebnisse in data/optitest/report_100coins.md speichern

Starten:
    python run_optitest.py
"""
...
from src.strategy_backtester import StrategyConfig, run_strategy_backtest_fast, run_strategy_backtest
...
OT_DIR    = os.path.join(os.path.dirname(__file__), "data", "optitest")
COIN_LIMIT     = 100
TIMEFRAME      = "15m"
DAYS           = 365
FEE            = 0.00055
POS_SIZE       = 0.05    # 5% pro Trade
TRAILING       = 0.008   # 0.8%
MIN_TRADES     = 15
LEVERAGES      = [1, 2, 3]
MIN_VOL_USDT   = 1_000_000    # Min. 1M USDT 24h-Volumen — filtert echten Mikroschrottheraus

# Explizite Blacklist: Meme-Coins, Scam-Tokens, 1000x-Derivate, illiquide Nischen
_MEME_BLACKLIST = {
    # 1000x-Derivate
...
def _is_quality_coin(symbol: str, vol_usdt: float) -> bool:
    """True wenn der Coin die Qualitätskriterien erfüllt."""
...
def fetch_top_symbols(limit: int) -> list[str]:
    log.info("Bybit Symbole laden (Top %d qualitative USDT-Perps)…", limit)
    ...
    # Ticker-Volumen laden
    try:
        tickers = ex.fetch_tickers(candidates[:600])
    except Exception as e:
        log.warning("Ticker-Laden fehlgeschlagen (%s) – alphabetisch ohne Volumen-Filter", e)
        tickers = {}
...
def download_missing(symbols: list[str]) -> list[str]:
    ...
    for i, sym in enumerate(symbols):
        p = csv_path(sym)
        if os.path.exists(p):
            log.info("[%d/%d] ✓ %s (gecacht)", i + 1, len(symbols), sym)
            ok.append(sym)
            continue
        try:
...
def write_report(results: list[dict], symbols_total: int, elapsed: float) -> str:
    ...
    lines = []
    lines.append(f"# OptiTest — 100-Coin Backtest Report\n")
    lines.append(f"**Datum:** {datetime.now().strftime('%Y-%m-%d %H:%M')}  ")
    lines.append(f"**Laufzeit:** {elapsed/60:.1f} min  ")
    lines.append(f"**Timeframe:** 15m · 365 Tage · Bybit Perps  ")
    ...
    lines.append("\n---\n")
    lines.append("## Top-20 Coins (nach PnL)\n")
    ...
    lines.append("```json")
    lines.append(json.dumps(results, indent=2, default=str)[:8000])
    lines.append("```")

    report = "\n".join(lines)

    report_path = os.path.join(OT_DIR, "report_100coins.md")
...
if __name__ == "__main__":
    ...
    log.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    log.info("  OptiTest 100-Coin Backtest")
    log.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")

    # 1. Symbole holen
    symbols = fetch_top_symbols(COIN_LIMIT)
    log.info("Symbole: %s", symbols[:10])

    # 2. Fehlende Downloads
    log.info("\n── Download fehlender Coins ──")
    ...
    log.info("  Report: data/optitest/report_100coins.md")
    log.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
